...
To ensure efficiency, keep the recommended number of classes within the range of 5 to 15.
adjust the recommendation gradually by increasing or decreasing the number by only 1 to 2 classes at a time to maintain consistency and prevent abrupt shifts.
...
Maintain the number of recommended classes within an appropriate range (e.g., minimum of 5 and maximum of 15).
Adjust the number of recommended classes gradually, increasing or decreasing by only 1 or 2 at a time, to avoid sudden changes
...
Now, proceed with generating the updated single-line debugging guidance about the number of classes to recommend as simple as possible if an update is required."""
        ...
        return "Recommend around 12 classes"


def update_stage_2(project_name,logs, memory_input_path, memory_output_path, answer_path_list,verbose):

    project_path = f"/home/##/ttr/mem/data/{project_name}/"

    summary_file_path = f"{project_path}summary.txt"
    basic_info_path = f"{project_path}memory/basic_info/Stage_2_info_0.txt"
